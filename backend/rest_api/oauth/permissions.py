...
class IsErrorDescriptionManager(permissions.BasePermission):
    """
    Custom permission to check if the authenticated user has permission to manage error descriptions.

    Assumes that experiment membership is populated during OAuth token processing.
    """

    def has_permission(self, request, view):
        return request.user.is_authenticated and request.user.has_perm("rest_api.can_manage_error_desc")
